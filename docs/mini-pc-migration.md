...
- Production `VITE_OURA_API_URL=/storage` uses a same-origin Vercel rewrite for the mini PC's public document API and change stream. This avoids browser local-network permission failures when Tailscale's DNS resolves the host to a tailnet address. Vercel relays these requests; SQLite on the mini PC owns all stored data. The browser does not initialize Firebase in this mode. Never expose the transport token in a `VITE_` variable.
