...
### Offline compaction of an existing backlog

For a large existing backlog, stop the API, timers and workers and record the time after all have stopped. After at least 15 minutes without readers or writers, run `node operator.mjs compact-records <new-database-path> <recorded-stop-time>` with the usual private configuration. This command creates a new database and never replaces the original. It keeps the current published Records graph, drafts and peer metric inputs; retains every other current document, original revision, inbox entry, migration record and unknown table; and preserves schema and autoincrement high-water marks. It rejects new writes after the recorded stop time, holds a source writer lock while copying, verifies retained rows with per-table counts and SHA-256 digests, and runs SQLite integrity checks. The manifest records the two intentional cleanup-control resets. Destination paths must be new and the filesystem must retain a 5 GiB reserve.

Only after the command succeeds should an operator switch the stopped service to the verified file. Keep the checkpointed original available for rollback until the replacement is serving correctly and an independent backup has passed its hash and integrity checks. Then remove only that explicitly retained, superseded working copy. Restart all normal timers but keep Firestore copy disabled. The backup-specific systemd drop-in `oura-worker@backup.service.d/timeout.conf` gives backup, hashing and verification two hours; sync jobs retain their three-minute timeout. Install this drop-in and reload the user systemd manager before verifying the backup.
