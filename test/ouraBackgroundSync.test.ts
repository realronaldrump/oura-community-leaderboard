...
describe("server-owned Oura snapshot model", () => {
  it("merges overlapping daily data and keeps the newest server value", () => {
    ...
    expect(merged.activity).toEqual([
      { id: "activity", day: "2026-08-09", steps: 4_000 },
    ]);
  });
  ...
  it("publishes a bounded launch snapshot without high-volume samples or diagnostics", () => {
    ...
    const snapshot = buildBackgroundDashboardSnapshot(
      "profile-1",
      {
        ...empty(),
  ...
  it("backfills one bounded history chunk per daily safety pass", () => {
    expect(
      getHistoryReconciliationRange("2026-01-01", "2016-01-01", 180),
    ).toEqual({
      startDay: "2025-07-05",
      endDay: "2025-12-31",
    });
...
describe("collection-specific coverage repair", () => {
  ...
  it("finds an unscanned gap inside already stored history and retries an optional failure", async () => {
    ...
    const recent = updateSourceCoverage(
      undefined,
      { startDay: "2026-09-01", endDay: "2026-09-10" },
      "ready",
    );
    expect(nextCoverageGap({ sleep: recent })?.endDay).toBe("2026-08-31");
    const older = updateSourceCoverage(
      recent,
      { startDay: "2016-01-01", endDay: "2026-08-31" },
      "ready",
    );
    const failed = updateSourceCoverage(
      recent,
      { startDay: "2026-03-05", endDay: "2026-08-31" },
      "failed",
    );
    expect(nextCoverageGap({ sleep: older, stress: failed })?.endDay).toBe(
      "2026-08-31",
    );
    const tomorrow = updateSourceCoverage(
      older,
      { startDay: "2026-09-10", endDay: "2026-09-11" },
      "ready",
    );
    expect(historicalCoverageKey({ sleep: tomorrow })).toBe(
      historicalCoverageKey({ sleep: older }),
    );
  });
});
